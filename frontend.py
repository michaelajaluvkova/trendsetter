...
Thanks!
""")
# --- Digitalization Toggle ---
#digitalize = st.radio("Digitalization Mode? (Digitalization mode saves history - keep uploading your MtG cards, we keep saving the card names and their counts. The result will be all your cards in single table.)", ["No", "Yes"])
digitalize = st.toggle("Digitalization Mode", value=False)


# --- Initialize session state for Digitalization ---
if digitalize == True:
    if 'collection_df' not in st.session_state:
        st.session_state['collection_df'] = pd.DataFrame(columns=['card_name', 'count'])
...
if uploaded_file:
    ...
    df_new = pd.DataFrame(deck, columns=['card_name', 'count'])
    ...
    if not deck:
        ...
        if digitalize == True:
            st.session_state['collection_df'] = (
                pd.concat([st.session_state['collection_df'], df_new])
                .groupby('card_name', as_index=False)
                .sum())

            st.session_state['collection_df'].sort_values(by="count", ascending=False, inplace=True)
            st.session_state['collection_df'].reset_index(drop=True, inplace=True)

            unique_cards = st.session_state['collection_df']['card_name'].nunique()
            st.success(f"{unique_cards} cards added to your collection.")

            #st.success("Cards added to your collection.")
            st.dataframe(st.session_state['collection_df'])
            csv = st.session_state['collection_df'].to_csv(index=False)
            st.download_button("Download Collection CSV", csv, file_name="my_card_collection.csv")
...
# KEEP_ALIVE_TIMESTAMP: 2026-02-27T01:44:23Z
